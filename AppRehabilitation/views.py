    ...
    if request.method == 'POST':
        ...
        with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
            while cap.isOpened():
                ...
                cv2.imshow('Mediapipe Feed Prueba', resized_image)
...
def rutina(request,numero_rutina):
    paciente = Paciente.objects.get(usuario=request.user)
    tipo_lesion = paciente.tipo_lesion
    context = {
        'tipo_lesion': tipo_lesion,
        'numero_rutina': numero_rutina
    }
...
class RutinaLuxacionConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        numero_rutina = query_string.split('=')[1]
        paciente = await sync_to_async(Paciente.objects.get)(usuario=user)
        rutina = await sync_to_async(Rutina.objects.get)(paciente=paciente, numero_rutina=numero_rutina)
        repeticiones = rutina.repeticiones_asignadas

        repeticion = await sync_to_async(Repeticiones.objects.create)(rutina=rutina, numero_repeticion=0)
        ...
        with self.mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as self.pose:
            ...
            while cap.isOpened():
                ...
                try:
                    lado = paciente.tipo_lesion
                    landmarks = results.pose_landmarks.landmark
                    hip = [landmarks[self.mp_pose.PoseLandmark.RIGHT_HIP.value].x if lado == 'Luxación del hombro izquierdo' else landmarks[self.mp_pose.PoseLandmark.LEFT_HIP.value].x,
                    landmarks[self.mp_pose.PoseLandmark.RIGHT_HIP.value].y if lado == 'Luxación del hombro izquierdo' else landmarks[self.mp_pose.PoseLandmark.LEFT_HIP.value].y]

                    hombro = [landmarks[self.mp_pose.PoseLandmark.RIGHT_SHOULDER.value].x if lado == 'Luxación del hombro izquierdo' else landmarks[self.mp_pose.PoseLandmark.LEFT_SHOULDER.value].x,
                            landmarks[self.mp_pose.PoseLandmark.RIGHT_SHOULDER.value].y if lado == 'Luxación del hombro izquierdo' else landmarks[self.mp_pose.PoseLandmark.LEFT_SHOULDER.value].y]

                    codo = [landmarks[self.mp_pose.PoseLandmark.RIGHT_ELBOW.value].x if lado == 'Luxación del hombro izquierdo' else landmarks[self.mp_pose.PoseLandmark.LEFT_ELBOW.value].x,
                            landmarks[self.mp_pose.PoseLandmark.RIGHT_ELBOW.value].y if lado == 'Luxación del hombro izquierdo' else landmarks[self.mp_pose.PoseLandmark.LEFT_ELBOW.value].y]

                    muneca = [landmarks[self.mp_pose.PoseLandmark.RIGHT_WRIST.value].x if lado == 'Luxación del hombro izquierdo' else landmarks[self.mp_pose.PoseLandmark.LEFT_WRIST.value].x,
                            landmarks[self.mp_pose.PoseLandmark.RIGHT_WRIST.value].y if lado == 'Luxación del hombro izquierdo' else landmarks[self.mp_pose.PoseLandmark.LEFT_WRIST.value].y]

                    indice = [landmarks[self.mp_pose.PoseLandmark.RIGHT_INDEX.value].x if lado == 'Luxación del hombro izquierdo' else landmarks[self.mp_pose.PoseLandmark.LEFT_INDEX.value].x,
                            landmarks[self.mp_pose.PoseLandmark.RIGHT_INDEX.value].y if lado == 'Luxación del hombro izquierdo' else landmarks[self.mp_pose.PoseLandmark.LEFT_INDEX.value].y]
                    ...
                    if stage is None:
                        if 5 <= angle_hombro <= 15 and 165 <= angle_codo <= 180:
                            ...
                            stage = "Inicial"
                            mensaje = "Levante el brazo"
                            fase_realizada = await sync_to_async(Fase.objects.create)(
                                repeticion=repeticion,
                                nombre_fase='Inicial',
                                angulo_brazo=angle_hombro,
                                angulo_codo=angle_codo,
                            ...
                            await sync_to_async(fase_realizada.save)()
                            print("Fase Inicial Realizada y Guardada con Exito")
                            stage2 = None      
                    ...
                    if stage == "Fase 2":
                        if angle_hombro > 90:
                            ...
                            if counter == repeticiones:
                                ...
                                rutina.is_completed = True
                                rutina.fecha_fin = datetime.now(tz=pytz.UTC)
                                await sync_to_async(rutina.save)()
                                cv2.rectangle(image, (200, 160), (400, 220), (0, 0, 0), -1)
                                cv2.putText(image, " Bien hecho!", (200, 200), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2, cv2.LINE_AA)
                                ...
                                await self.send(text_data=json.dumps({
                                    'image': image_data,
                                    'redirect': '/ejercicios/rutina/'
                                }))

                                
                                break
                            else:
                                repeticion = await sync_to_async (Repeticiones.objects.create)(rutina=rutina, numero_repeticion=repeticiones_completadas)
    ...
    def disconnect(self, close_code):
        # Liberar recursos cuando se cierra la conexión WebSocket
        self.pose.close()

class RutinaLesionMediaConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        await self.accept()
        ...
        with self.mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as self.pose:
            ...
            while cap.isOpened():
                ...
                try:
                    ...
                    hip_index = self.mp_pose.PoseLandmark.LEFT_HIP.value if lado == 'Lesión media en el hombro derecho' else self.mp_pose.PoseLandmark.RIGHT_HIP.value
                    hombro_index = self.mp_pose.PoseLandmark.LEFT_SHOULDER.value if lado == 'Lesión media en el hombro derecho' else self.mp_pose.PoseLandmark.RIGHT_SHOULDER.value
                    codo_index = self.mp_pose.PoseLandmark.LEFT_ELBOW.value if lado == 'Lesión media en el hombro derecho' else self.mp_pose.PoseLandmark.RIGHT_ELBOW.value
                    muneca_index = self.mp_pose.PoseLandmark.LEFT_WRIST.value if lado == 'Lesión media en el hombro derecho' else self.mp_pose.PoseLandmark.RIGHT_WRIST.value
                    indice_index = self.mp_pose.PoseLandmark.LEFT_INDEX.value if lado == 'Lesión media en el hombro derecho' else self.mp_pose.PoseLandmark.RIGHT_INDEX.value

                    hip = [landmarks[hip_index].x, landmarks[hip_index].y]
                    hombro = [landmarks[hombro_index].x, landmarks[hombro_index].y]
                    codo = [landmarks[codo_index].x, landmarks[codo_index].y]
                    muneca = [landmarks[muneca_index].x, landmarks[muneca_index].y]
                    indice = [landmarks[indice_index].x, landmarks[indice_index].y]
    ...
    def disconnect(self, close_code):
        # Liberar recursos cuando se cierra la conexión WebSocketa
        cv2.destroyAllWindows()
        self.pose.close()